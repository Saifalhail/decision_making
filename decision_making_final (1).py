...
def getCost(situationlist, situationqueue, individual):
    ...
    # have added tracking already allotted units
    alloted_units = {}
    situation_allotted = []
        ...
        for elem in situationqueue[key]:
            ...
            for unit in listofavailableunits:
                ...
                if elem.situation in unit.capability:  # Checks if vehicle fits the scenario required
                    ...
                    # earlier we were not using information about allotted units to different situations
                    num_capable_units += min(
                        sum(alloc[unit.asset][0]) + sum(alloc[unit.asset][1]) - alloted_units[unit.asset],
                    ...
                    if elem.solution in unit.effectiveness:
                        num_effective_units += min(
                            sum(alloc[unit.asset][0]) + sum(alloc[unit.asset][1]) - alloted_units[unit.asset],
                            elem.numUnits)
                        # we allocate units after storing time data for all the unit allocations for all situations
                        ...
                        situation_allotted.append({unit.asset: min(
                            sum(alloc[unit.asset][0]) + sum(alloc[unit.asset][1]) - alloted_units[unit.asset],
                            elem.numUnits), 'penalty_A': time_penalty_A, 'penalty_B': time_penalty_B})

                        alloted_units[unit.asset] += min(sum(alloc[unit.asset][0]) + sum(alloc[unit.asset][1]) -
                                                         alloted_units[unit.asset], elem.numUnits)


                else:
                    if (sum(alloc[unit.asset][0]) + sum(alloc[unit.asset][1]) - alloted_units[unit.asset]) > 0:
                        cost += HARD_CONSTRAINT_PENALTY * HARD_CONSTRAINT_PENALTY * (
                                    sum(alloc[unit.asset][0]) + sum(alloc[unit.asset][1]) - alloted_units[unit.asset])
                        alloted_units[unit.asset] += (
                                    sum(alloc[unit.asset][0]) + sum(alloc[unit.asset][1]) - alloted_units[unit.asset])
    ...
    # select most efficient allocation
    situation_allotted = pd.DataFrame(situation_allotted)
    situation_allotted['gaps'] = situation_allotted['penalty_A'] - situation_allotted['penalty_B']

    # using the gap when choosing A over B - we select the ones having the least gap and then select ones having most gap from station B
    for unit in listofavailableunits:
        situation_allotted.sort_values(by='gaps', ascending=True, inplace=True)
        situation_allotted.reset_index(drop=True, inplace=True)
        cost += situation_allotted['penalty_A'].iloc[0:sum(alloc[unit.asset][0])].sum() * SOFT_CONSTRAINT_PENALTY
        situation_allotted.sort_values(by='gaps', ascending=False, inplace=True)
        situation_allotted.reset_index(drop=True, inplace=True)
        cost += situation_allotted['penalty_B'].iloc[0:sum(alloc[unit.asset][1])].sum() * SOFT_CONSTRAINT_PENALTY
        situation_allotted.reset_index(drop=True, inplace=True)
